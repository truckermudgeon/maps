...
| Where do MapLibre calls happen?                                  | `services/map-adapter.ts` (the single maplibre boundary — see its docs)      |
...
3. **`MapAdapter` is the single maplibre boundary.** Anything that
   ...
   belongs there. Callers get high-level methods (`setPlayerPose`,
   `beginFollowCamera`, `fitPoints`, `flyTo`); they should not see
   `Marker` or `LngLat` types. (`route-renderer.ts` currently reaches
   through `getMap()` to call `setSourceData` / `setPaintProperty` —
   the one known gap; either grow `MapAdapter` or admit `RouteRenderer`
   as a second boundary file. Tracked as a follow-up.)
...
4. **Reactions bridge stores to services.** When a store change should
   ...
   stores → reactions → services. `MapAdapter.isMapLoaded` is exposed
   as the one observable on the adapter so reactions can replay
...
src/
  ...
  services/           ← IO, DOM, lifecycle:
                          MapAdapter        — single maplibre boundary
...
1. Add a method on `MapAdapter` if it touches maplibre. Otherwise reach
   for an existing service or create a new one.
...
- `RouteRenderer` reaches into the map via `MapAdapter.getMap()` to
  call `setSourceData` / `setPaintProperty`. Either grow `MapAdapter`
  with thin wrappers and drop `getMap()`, or update rule #3 to admit
  the renderer as a second maplibre boundary file.
